...
public class QueueCtrl {
    ...
    public static PatientQueue findByPatientId(Context context, int patientId) {
        var tempQueue = new LinkedQueue<PatientQueue>();
        PatientQueue found = null;

        while (!context.queue.isEmpty()) {
            var current = context.queue.dequeue();
            if (current.getPatient().getId() == patientId) {
                found = current;
            }
            tempQueue.enqueue(current);
        }
    ...
    public static void completePatient(Context context, int patientId) {
        var pq = findByPatientId(context, patientId);
        if (pq != null) {
