...
public class AddConsultationUI extends UI<Context> {
    ...
    protected UI<Context> display() {
        ...
        var doctorID = readString("Enter Doctor ID (e.g. D001): ",
                input -> context.doctors.has(input.strip()) ? null : "Doctor not found.").strip();
        ...
        var doctor = context.doctors.get(doctorID);
        ...
        readString("Enter Date and Time (" + DATE_FORMAT + "): ", input -> {
            ...
            if (!ConsultationCtrl.isDoctorAvailable(context, doctor, dateTime))
        ...
        context.consultations.add(new Consultation(consultationID, patient, doctor, dateTime, diagnosis, remarks));
